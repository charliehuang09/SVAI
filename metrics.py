...
def main():
    ...
    preds = np.nanmean(np.nanmean(preds, axis=2), axis=1)
    ground_truth = np.nanmean(np.nanmean(ground_truth, axis=2), axis=1)
    ...
    for i in range(len(predsTrain)):
        ...
        ax0img = ax[0].imshow(predsTrain[i], cmap=plt.cm.get_cmap("BrBG"), vmin=min_, vmax=max_)
        ax[0].set_title("Predictions")
        
        ax1img = ax[1].imshow(ground_truth_Train[i], cmap=plt.cm.get_cmap("BrBG"), vmin=min_, vmax=max_)
        ax[1].set_title("Ground Truth")
        
        plt.colorbar(ax0img, ax=ax.ravel().tolist()[:2]) 
        
        ax2img = ax[2].imshow(abs(ground_truth_Train[i] - predsTrain[i]), cmap=plt.cm.get_cmap("CMRmap"), vmin=min_, vmax=max_)
    ...
    for i in range(len(predsValid)):
        ...
        ax0img = ax[0].imshow(predsValid[i], cmap=plt.cm.get_cmap("BrBG"), vmin=min_, vmax=max_)
        ax[0].set_title("Predictions")
        
        ax1img = ax[1].imshow(predsValid[i], cmap=plt.cm.get_cmap("BrBG"), vmin=min_, vmax=max_)
        ax[1].set_title("Ground Truth")
        
        plt.colorbar(ax0img, ax=ax.ravel().tolist()[:2]) 
        
        ax2img = ax[2].imshow(abs(ground_truth_Valid[i] - predsValid[i]), cmap=plt.cm.get_cmap("CMRmap"), vmin=min_, vmax=max_)
        ax[2].set_title("Difference")
